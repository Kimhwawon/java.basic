...
public class Warrior {
	...
	public void huntTroll(Troll troll) {
		troll.doBattle(this.atk); //츠롤이 전사가 공격하는 로직
		if(troll.getHp() > 0 ) { // 트롤 체력이 0보다 큰지 검사(0보다크다면)
			this.hp = this.hp + this.def - troll.getAtk() ; //정상 교전 진행
		}else { // 0보다 작거나 같다면
			System.out.println("프롤이 이미 죽어서 교전이 긑났습니다."); // 트롤이 죽었음을 알림
		}
		
		System.out.println("교전 결과 전사의 체력 : " + this.hp); // 교전결과 브리핑
	}
	
	
	// getter/setter/ Warrior, Orc에 만들어주세요.
