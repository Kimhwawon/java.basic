...
//구매자 Buyer
public class Buyer {
// 구매자는 int money(소지금액), int mango(소지망고개수)를 멤버 변수로 가집니다.
	private int money; // 소지금
	private int mango; // 소지망고개수
// 생성자에 돈을 받아서 소지금액을 초기화 해주고 망고는 0으로 초기화합니다.
...
// 판매자의 망고가 내가 구매하려는 망고보다 적은경우도 역시 "망고재고가 부족합니다"
	if(seller.getmango() < mango) {
		System.out.println("망고재고가 부족합니다. 망고재고 : " + seller.getmango());
	return;
	}
...
// seller의 망고를 sellMango()가 호출되도록 처리하며
// seller의 망고를 차감하고 돈을 올려주는 로직
seller.sellMango(mango);
}

//shoeBuyer()를 이용해 소지 망고, 소지금을 볼 수 있습니다.
