...
public class Lotto03 {
	...
	public static void main(String[] args) {
		...
		// 로또번호를 받을 수 있는 변수 생성 0으로 초기화
		int getNum = 0;
		
		// while문 조건문 번홀를 담은 ArraysList의 내부 요소 개수가 6이 되면 종료되도록
		while (lotto.size() != 6) {
			// 번호뽑기
			...
			// 저의경우는 반복문 진입 전에 int count = 0;을 만들어 놓고
			int count = 0;
			...
			while(!lotto.equals(luckyNum)) {
				...
				// 다 돌면 정렬하기
					...
					// 그러므로 모든 로직이 다 끄탄고 마지막에 1 더해줌
					count++;
						
		
			}
			// 반복문이 몇 바퀴 돌았는지 카운팅하는 로직을 위해 추가해주세요.
			// 저의경우는 반복문 진입 전에 int count = 0;을 만들어 놓고
			// 반복문이 한 자퀴 돌 떄 마다 count++; 를 이용해 1씩 증가시켜놓고
			// 탈출 후에 콘솔에 최종횟수만 찍히도록 합니다. 
