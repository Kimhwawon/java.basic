...
테이블 내의 하나의 컴럼은 반드시 primary key 속성을 가져야 합니다. 
...
만약 모든 컬럼을 대상으로 지정시 컬럼은 생략하고 VALUES이후 값만 지정할 수 있다. 
INSERT INTO 테이블명 VALUES(컬럼1값, 컬럼2값, ... ); */
INSERT INTO users (id, pw, user_name, email) VALUE 
('abc1324', 'asdf', '김자바', 'asdf@asdf.com');
...
INSERT INTO users VALUES ('qwer123', 'qwer', '쿼리문', 'query@query.com');
 ...
 INSERT INTO users VALUES ('aaa132', 'aaaa', '김에이','aaaa@aaaa.com');
 INSERT INTO users VALUES ('bbb123', 'bbbb', '김비', 'bbbb@bbbb.com');
 INSERT INTO users VALUES ('ccc123', 'cccc', '김씨', null);
 ...
 -- 아이디가 xxxx인 사람만 출력해보세요.
 SELECT * FROM users WHERE id = 'aaa132';
 ...
 SET SQL_SAFE_UPDATES =0; -- 0이면 비활성화 1로주면 sefety모드 활성화 
 SELECT * FROM USERS;
 
 -- WHERE구문을 이용해서 pw컬럼의 값을 모두 다르게 바꿔주세요.
 UPDATE users SET pw = ' pwww ' WHERE id = 'aaa132';
 UPDATE users SET pw = ' 0123 '  WHERE id = 'bbb123';
  UPDATE users SET pw = '1234 ' WHERE id = 'ccc123';
  UPDATE urers  SET pw = 'oracle', user_name = '오라클유저' WHERE id = 'mysqlsql';
  -- 여러분들이 원하는 로우(row) 한에 대해서 이름과 비밀번호를 함께 바꿔주세요. 
  UPDATE users SET pw = '960406' , user_name = ' 멕시코 ' WHERE id = 'bbb123';
  
  -- 데이터 삭제
  /* DELETE FROM 테이블명; 을 할 경우 테이블 전체 데이터가 싹 다 날아갑니다.alter
  DELETE구문 역시 WHERE과 조합해서 사용해야 합니다. */
  DELETE FROM users WHERE id = 'aaa132';
...
INSERT INTO library VALUES ('빨간책', 1000, '빨간', '김빨간', 235);
INSERT INTO library VALUES ('노랑책', 2000, '노랑', '김노랑', 335);
INSERT INTO library VALUES ('파랑책', 3000, '파랑', '김파랑', 435);
INSERT INTO library VALUES ('보라책', 4000, '보라', '김보라', 535);
INSERT INTO library VALUES ('검정책', 5000, '검정', '김검정', 635);
INSERT INTO library VALUES ('초록책', 6000, '초록', '김초록', 735);
SELECT * FROM library;
